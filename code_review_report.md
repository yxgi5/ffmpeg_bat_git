...
| ✅ 已确认 | **`-map 0:v` 保持严格、不加 `?`（设计决策，附实测对照）**：纯音频输入实测 —— `-map 0:v -map 0:a?` → ffmpeg 报 `Stream map '' matches no streams` + `Failed to set value '0:v'`，无产物、非零退出；改 `-map 0:v?` → **rc=0 且静默产出只有音轨的 mp4**（对视频压缩工具而言是"错误的成功"）。故 `0:v` 作为工具目的本身应当失败而非降级，`?` 只给真正可选的 `0:a?`（无音轨的录屏/延时摄影是合法输入，且当时无其他守卫）与 `0:s?`。全仓 14 处 `-map 0:v -map 0:a? -map 0:s?` 完全一致。**疑点已由第六轮确证为真缺陷**：`%SRC_FILE:&=^&%` 里路径已被双引号包裹，而 cmd 的 `^` **在双引号内是字面量**，替换结果因此是 `"…\Fast ^& Furious.mp4"` —— 脱字符被写进了真实路径，ffprobe/ffmpeg 必然找不到文件。详见下一行 | 本次核查 |
| ✅ 已解决 | **路径元字符全链路加固 —— 片库确有含 `&` 的片名（用户反馈触发）**。原先所有「路径安全」只覆盖到空格：引号保护了空格与 `&` 的分词，但**引号内的 `^` 不再是转义符**，「转义 `&`」这个动作本身反而污染了路径。<br>**① `^` 转义反噬（P1，原版即有）**：`%SRC_FILE:&=^&%` 共 **33 处**（4 编码器 × 8 + copy_to_mp4 × 1）在双引号内生效 → 含 `&` 的片名被送进一个字面含脱字符的路径。全部改回 `%SRC_FILE%`（引号已足够）。<br>**② `%1` 裸展开（P1）**：`if [%1] neq []` + `SET SRC_FILE=%1` —— 拖放**含 `&` 但无空格**的片名时 Explorer 不加引号，`&` 在 if/set 语句里直接当命令分隔符断行（且 `%1` 含 `(`/`)` 还会破坏 `if` 语法）。改 `if not "%~1"==""` + `set "SRC_FILE=%~1"`（去引号，且展开落在双引号内）。<br>**③ RUN_COM 拼接与执行（P1）**：`SET RUN_COM=%RUN_COM% %TARGET_FILE%` 非 `set "VAR=…"` 形式 → 值里的 `&`/`)` 断行；末尾 `call %RUN_COM%` 让整条命令行**再过一次 `%` 展开**（call 的二次解析），片名含 `%x%` 形态会被当变量吃掉。已改 `set "RUN_COM=…"` + **直接执行**（去掉 `call`），输出路径另加一步统一补引号。交互分支判定 `IF not [%1] NEQ []` 一并改写为严格等价的 `IF "%~1"==""`（原写法在 `%1` 含 `(`/`)`/`&` 时语法就会被破坏）。<br>**④ list 驱动 bat 开着无用的延迟展开（P1）**：4 个 `convert_from_list_*` / `repack_from_list` 首行 `setlocal EnableDelayedExpansion`，但脚本里没有任何 `!…!` —— 反而让 `for /f` 变量 `%%i` 里的感叹号被成对吃掉，`Tora! Tora! Tora!.mp4` 会变成残缺路径。改 `DisableDelayedExpansion`。<br>**⑤ 编码器延迟展开块内含路径（P2）**：`%SRC_RESOLUTION%` 的执行原先落在 `setlocal EnableDelayedExpansion` 之后，而块内 `%VAR%` 的**展开结果还要再被延迟展开扫一遍**（同一类 `!` 问题）。已把该行移到块外执行。<br>**验证（第六轮探针 `smoke_special_chars.bat`，待跑）**：四部分 —— **A** 20 个文件名字符用例（`&`/`%`/`!`/`( )`/`[ ]`/`^`/`;`/`,`/`=`/`#`/`$`/`+`/`'`/`~`/`@` + 真实片名形状 `A & B (2020)`、`Tora! Tora! Tora! (1970)`、`A&B!C(2) 100%` + 子目录 `sub & dir (x)`）走 `ffmpeg_copy_to_mp4`；**C** 完整编码路径 `ffmpeg_avc_qsv` 跑三个真实片名形状；**B** `convert_from_list_qsv` 清单驱动 6 条含元字符条目；**D** 无参数模式（双击后手输路径）用 stdin 喂含 `&` 的路径。<br>**已知边界（本次刻意未改，避免引入未验证的改动）**：`call` 传参链路（`extract`/`get_suffix`/`check_isvideo`）仍会二次解析 `%`，片名出现 `a%b%c` 这种**成对百分号且中间是合法变量名**的形态仍会失败（`100% Wolf.mp4` 这种单 `%` 安全）；`lookup_bitrate` 延迟块内仍是 `%LB_FILE%`，故「仓库自身路径含 `!`」不受支持 —— 两者都与片库路径无关 | (本次) |
...
**遗留事项**：.bat 家族重构与后续多轮冒烟已完成（T1–T13 全绿）；**第六轮路径元字符加固待跑探针 `smoke_special_chars.bat` 复核**；Linux（A 机）与 Ultra 265K（C 机）平台实测（见 environment_matrix.md）；README/使用说明。
