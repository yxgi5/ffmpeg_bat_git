...
**遗留事项**：.bat 家族重构与 T1–T13 冒烟已完成；**第六轮 6b 的 `set` 写法修正待跑探针 `smoke_special_chars.bat`（v6b）复核**；Linux（A 机）与 Ultra 265K（C 机）平台实测（见 environment_matrix.md）；README/使用说明。
