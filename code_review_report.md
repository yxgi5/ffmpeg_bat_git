...
另修正两点：A 机 **`/opt/ffmpeg` 未安装**（master 行对其不适用）；**`libx264`/`libx265` 在 Linux 侧打包内即有 → 「软编保底必须走 mingw64」对 Linux 不成立**。
