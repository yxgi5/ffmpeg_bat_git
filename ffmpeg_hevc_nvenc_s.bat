...
::set "RUN_COM="%FFMPEG_PATH%" -hide_banner -threads 0 -hwaccel cuvid -hwaccel_output_format cuda"
::set "RUN_COM="%FFMPEG_PATH%" -hide_banner -threads 0 -v verbose -hwaccel auto"
set "RUN_COM="%FFMPEG_PATH%" -hide_banner -threads 0 -v verbose -init_hw_device cuda=hw -filter_hw_device hw"
...
::pause
::echo RUN_COM0:%RUN_COM%
::if defined BIT set "RUN_COM=%RUN_COM:&=^&% -c:v hevc_nvenc -profile:v main -preset p4 -tune:v hq -rc cbr -b:v %SRC_BITRATE% -fps_mode cfr -pix_fmt yuv420p -color_range tv -colorspace bt709 -color_primaries bt709 -color_trc bt709 -g 250 -keyint_min 25 -sws_flags bicubic -ar 44100 -b:a 128k -c:a aac -ac 2 -map_metadata -1 -map_chapters -1 -strict -2 -rtbufsize 120m -max_muxing_queue_size 1024"
if defined BIT set "RUN_COM=%RUN_COM:&=^&% -c:v hevc_nvenc -profile:v main -preset p4 -tune:v hq -rc cbr -b:v %SRC_BITRATE% -fps_mode cfr -color_range tv -colorspace bt709 -color_primaries bt709 -color_trc bt709 -g 250 -keyint_min 25 -ar 44100 -b:a 128k -c:a aac -ac 2 -map_metadata -1 -map_chapters -1 -strict -2 -rtbufsize 120m -max_muxing_queue_size 1024"
