...
> **第六轮（2026-09-16，路径元字符加固）**：用户反馈**片库确有含 `&` 的片名**，遂做一次全链路元字符审计，确证并修复 5 类缺陷（逐条见 code_review_report.md 对应行）：① `%SRC_FILE:&=^&%`（33 处）的 `^` 落在**双引号内是字面量** → 脱字符被写进真实路径；② `if [%1] neq []` / `SET SRC_FILE=%1` 让 `%1` 裸展开 —— 拖放含 `&` 且**无空格**的片名时 cmd 在此断行；③ `SET RUN_COM=…`（非 `set "VAR=…"`）+ `call %RUN_COM%` 让值里的 `&` 断行、`%` 被二次解析；④ 4 个 list 驱动 bat 无用却开着 `EnableDelayedExpansion`，`for /f` 变量里的 `!` 被成对吃掉；⑤ 编码器 `%SRC_RESOLUTION%` 的执行落在延迟展开块内（同一 `!` 问题），已移出。<br>**验证方式**：新增探针 `smoke_special_chars.bat`（放仓库外、纯 ASCII + CRLF，避免探针自身踩编码坑），四部分 —— **A** 20 个文件名字符用例走 `ffmpeg_copy_to_mp4`（`&` `%` `!` `( )` `[ ]` `^` `;` `,` `=` `#` `$` `+` `'` `~` `@`，外加真实片名形状 `A & B (2020)` / `Tora! Tora! Tora! (1970)` / `A&B!C(2) 100%` 与子目录 `sub & dir (x)`）；**C** 三个真实片名形状走完整编码路径 `ffmpeg_avc_qsv`；**B** `convert_from_list_qsv` 清单驱动 6 条含元字符条目；**D** 无参数模式（双击后手输路径）用 stdin 喂含 `&` 的路径。**待跑（阻塞项）**。<br>**已知边界（刻意未改）**：`call` 传参链路（`extract`/`get_suffix`/`check_isvideo`）仍会二次解析 `%`，片名若是 `a%b%c` 这种成对百分号形态仍会失败（`100% Wolf.mp4` 单 `%` 安全）；`lookup_bitrate` 延迟块内仍为 `%LB_FILE%`，故「仓库自身路径含 `!`」不受支持 —— 两者均与片库路径无关。
...
11. **`&` 文件名** → 第六轮已**确证为真缺陷并修复**（`^` 在双引号内是字面量，脱字符进了真实路径；33 处 `%SRC_FILE:&=^&%` 已改回 `%SRC_FILE%`），并顺带处理了 `%1` 裸展开、`RUN_COM` 拼接/`call` 二次解析、list bat 延迟展开、编码器延迟块内含路径共 5 类问题；**待跑探针 `smoke_special_chars.bat` 复核**（阻塞项，见上）
