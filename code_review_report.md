...
| ✅ 已解决 | **sh 族全覆盖补测 + 一键冒烟套件沉淀（2026-09-16 第二轮，merge 后静态复核驱动）**。sh 族 15 个入口中 2 个此前未测：① `convert_from_list_cuda.sh` 补测——Linux 分发打印 `Linux` → 清单条目完整读取 → 调 hevc_nvenc 预期失败 `Convert failed！` 传播 rc=1（复证 run_list stdin 修复）；② `ffmpeg_hevc_nvenc_cygwin.sh` 发现无执行位（git 100644，与 libx264 同款）→ `chmod +x` 修复（全仓复核：15 个入口脚本全 755，`lib/common.sh` 作为被 `source` 的库保持 644 合理），修复后直跑验证 `-hwaccel cuvid`+`hwdownload` 命令构建正确 → `CUDA_ERROR_NO_DEVICE` 预期失败 rc=1。至此后 sh 族 **15/15 入口全部有本机实测断言**。③ 探针整合沉淀为仓库外 `ffmpeg_bat_git_smoke/smoke_all.sh`（自生成素材 + 18 用例：7 编码入口查表断言 / 3 无卡预期失败 / copy_to_mp4 两态 / 3 list / 3 边界与降帧 + PASS/FAIL 汇总，与 smoke_all.bat 对称），实测两轮 **PASS=18 FAIL=0** | 本次 |
| ✅ 已完成 | **A 机（i7-9700T）Ubuntu 22.04 实测 —— PASS=22/22**。发行版 4.4.2 单构建下：`h264_vaapi` / **`hevc_vaapi`** / `h264_qsv` / `hevc_qsv` / `libx264` / `libx265` / `copy_to_mp4` 全部可用；`av1_qsv`·`*_nvenc` 按硬件缺失干净失败；交互模式（含码率覆盖 `900k`）2/2；list 模式 3/3（含带空格文件名）且 **CRLF+BOM 清单通过**；门禁 3/3；**`</dev/null` 回归 5 条目 `outputs=5/5`（跨机器确认）**。
**关键修正**：A 机与 C 机是**同版本 4.4.2 + 同版本 iHD 25.2.4**，C 机 `hevc_vaapi` ⛔（issue 24）而 **A 机 ✅** —— 报告原文把该失败归为「4.4.2 与新版 iHD 不兼容」**归因不完整**，实测两者走的是**同一条 legacy `VAEntrypointEncSlice` 路径、同一个 `Lavc58.134` 构建**，差异只在硬件代际 → 正确表述是「取决于具体核显代际」，C 机 issue 24 的成因**本轮不下结论、留待交叉验证**。
另修正两点：~~A 机 `/opt/ffmpeg` 未安装（master 行对其不适用）~~ → **当日下午 `/opt/ffmpeg` master-gpl 已到货**（`N-117740-g7f51cf75c6-20241110`，与 C 机同一构建），已补做**双构建轮**（见下）；**`libx264`/`libx265` 在 Linux 侧打包内即有 → 「软编保底必须走 mingw64」对 Linux 不成立**。
**双构建轮（2026-09-16 下午，`/opt` 到货后）**：两套 ffmpeg 各跑一遍同一套件，**均 PASS=22/22**；`real TARGET_BITRATE` 两轮一致、`</dev/null` 回归两轮均 `outputs=5/5`。**本轮最重要的机制定论：发行版 4.4.2 的 QSV 硬解是「静默回退软解」** —— 4 种源格式 × 3 种写法实测，4.4.2 侧**全部 rc=0 但滤镜像素格式仍是源格式**（`pixfmt:yuvj420p`/`yuv420p`），只有显式 `-hwaccel_device hw` 才报 `Device setup failed for decoder`；master 侧同批用例全部 `Selecting decoder 'h264_qsv'|'hevc_qsv'` + `pixfmt:qsv` = 真硬解。**由此推出的实用结论：装 `/opt` 并不改变整机默认（`which ffmpeg` 仍是 `/usr/bin/ffmpeg`），仅 `av1_qsv.sh`/`hevc_vaapi.sh`/`av1_nvenc.sh` 三个软偏好脚本会切换；4.4.2 上的 QSV 脚本实为「软解+硬编」（出片正确，只是 CPU 侧未省），要名实相符的全硬解链路需自行前置 `/opt/.../bin`。** 另：A 机新增可用软编 AV1（`libsvtav1`，master 独有）；AV1 硬编在 Gen9.5 仍不可用（master 里 `av1_qsv`/`av1_vaapi` 存在但打开编码器即失败 `-22`）。
**探测方法学（两条教训，已回写 environment_matrix.md）**：① **`ffmpeg -h encoder=<名>` 判断编码器是否存在会误判** —— 对不存在的编码器**同样返回 rc=0**，只打印 `Codec 'x' is not recognized by FFmpeg.`（本次首轮勘察因此得出"4.4.2 拥有全部编码器"的错误表格，用对照名 `definitely_not_a_codec` 校准后纠正）；正确做法是 grep `not recognized` 或直接试跑。② **解码是否真走硬件不能看耗时，要看日志**（`pixfmt:` 与 `Selecting decoder`）——4.4.2 不打印 `Selecting decoder`（FFmpeg 5.x 起才有），VAAPI 看 `pix_fmt: vaapi_vld`。
**剩余待验证**：A 机 Win11 的 QSV（UHD 630）；C 机 Win11 行（`.bat` 入口已备齐 7 个，含新增的 AV1 NVENC / AV1 QSV，探针 T14/T15 就位）。见 environment_matrix.md 待验证清单 | (本次) |
| ✅ 已完成 | **C 机 Linux 复查轮（2026-09-16 下午，SSH 远程复跑）+ `hevc_vaapi` 归因收口 + 新增 `ffmpeg_av1_nvenc.bat`**。① 同一套件在 C 机双构建各跑一遍：**均 PASS=22/22**；与 A 机唯一差异是 `ffmpeg_av1_qsv.sh` **真产出 AV1 产物**（Arrow Lake 有 AV1 硬编；A 机 Gen9.5 为预期失败）→ 套件期望值参数化为 `EXPECT_AV1_QSV=ok|fail`。② **查表跨机器一致**（1080p：AVC `3836249` / HEVC `2548951` / AV1 `1656818`，与 B 机 Windows 侧 `.bat` 探针 T1/T2 期望值**同值**；`h264_vaapi` 同为 `3836249`）→ 首轮修的「h264_vaapi 误用 HEVC 表」复测保持正确；1080p60 降帧分支三入口均正确输出 30fps。③ **归因收口**：借 `/opt/ffmpeg` 里既有的 gyan 静态包做第三方交叉验证 —— `hevc_vaapi` 在 **4.4.2 与 4.4.3 报同一错误（`Encode failed: -5`）**，**5.1.2 与 master 均 ✅** → 判定 **FFmpeg 4.4.x 的 VAAPI HEVC 老路径在 Arrow Lake 上失效，与 Ubuntu 打包无关，5.1 起修复**；附带：两个 gyan 静态包自带 `libsvtav1`（Ubuntu 4.4.2 没有 → 「4.4.2 无 svtav1」属**打包差异**），但它们用旧 libmfx API 对接 libvpl2 2.15.0 会**直接段错误**（`Error setting child device handle: -17` + core dumped），BtbN `7.0.2-static` 则是**纯软编构建**。④ QSV 硬解「静默软解」机制在 C 机复现 → **两机表述统一**（4.4.2 侧 rc=0 但 `pixfmt` 仍为源格式，显式 `-hwaccel_device hw` 才失败；master 侧 `pixfmt:qsv` + `Selecting decoder`）。⑤ 新增 **`ffmpeg_av1_nvenc.bat`** —— `.bat` 家族此前唯独缺 AV1 NVENC 入口（而 `lib/common.bat` 的 `lookup_bitrate` 早已把 `bitrate_table_av1.csv` 列为受支持表、`.sh` 侧 `ffmpeg_av1_nvenc.sh` 也已在 B 机实测通过），故以 `ffmpeg_hevc_nvenc.bat` 为骨架补齐（编码器段去掉 `-profile:v main`、码率表改 `bitrate_table_av1.csv`、标题与注释同步、CRLF 与 cp65001 守卫原样保留）；静态自检（`check_bat_static` / `audit_set_forms2` / `mini_cmd_scan`）结果与其骨架文件**完全一致**，**B 机 Windows 侧实跑验证待补（探针 T14，期望 `TARGET_BITRATE=1656818` 且产物 `v=av1`）**。⑥ 本轮另暴露两个探测陷阱并已入档：**日志首个 `Lavf` 是输入文件元数据而非运行中的二进制**（须取 `Output #0` 的 `encoder : Lavc`）、**`-vf` 必须置于 `-i` 之后**（放前面会得到 `Option vf cannot be applied to input`，看起来像"功能不可用"）。 | 本次 |

| ✅ 已完成 | **三层测试体系落地：静态/对等检查 + 冒烟对等化 + 环境能力报告（2026-09-16 晚）**。此前测试只有"冒烟"一层，且 sh 族是 22 个**扁平用例**（无编号、无 T 对应、SKIP 语义不同），与 bat 的 T 编号套件**形态上不对等** —— 两族无法逐条对照，改一边容易忘另一边。本轮补齐三层，职责不重叠：<br>**① 第一层 `test/lint/`（新增，秒级、确定性）**：`lint.py` 13 项静态检查（L01–L13）+ 7 项两族对等检查（P01–P07），最终 **`21 PASS / 0 FAIL / 7 WARN`**、退出码 0；`selftest.py` 13 个合成仓库用例做双向验证。<br>**② 第二层 `test/sh/smoke_sh.sh` v2（重写）**：与 `smoke_ffmpeg_bat.bat` **同 T 编号（T1–T25）、同断言粒度、同 SKIP 语义**，harness 真实返回退出码；本机实测 `PASS=22 FAIL=0 SKIP=4`。<br>**③ 第三层 `test/{sh,bat}/check_env.*`（新增）**：回答"**这台机器能跑哪些入口、为什么不能**" —— 默认秒级快查，`--probe` / `PROBE` 深测时每个候选真跑一遍 3 秒小片。统一状态词表 `OK / NO-ENCODER / NO-DEVICE / N/A-OS / UNKNOWN / PROBE-OK / PROBE-FAIL / NO-ENTRY`。<br>**7 条 WARN 全是数据观察、非代码缺陷**：`bitrate_table_avc.csv` 有 5 行与前一行完全重复（值相同、永不可达）；`bitrate_table_av1.csv` 第 51 行码率小幅回落 -1.7%；`libx265` preset `sh=fast / bat=veryfast` 属白名单内的有意差异。P04 因此拆成**硬约束**（`max_pixels` 不回退、同像素不映射两码率、720p/1080p 必须有覆盖 → FAIL）与**软观察**（码率回落、完全重复行 → WARN）。<br>**第三层的设计目标是让模糊态显式化**：正是它把"硬件在但测不到""硬件不在却报错"这类此前只能靠人猜的情况变成了可读的状态词（下一条即为实证）。文档落在 `test/README.md`（7 节：三层总览 / 目录约定 / L+P 检查表与 L09 机制 / T 编号对照表与 SKIP 策略 / 状态词表 / 对等矩阵 / 实测记录）。 | (本次) |
| ✅ 已完成 | **`lint.py` 的 5 类假阳性根因定位 + L09 重设计为"差分式扫描"（"只会输出全部干净的检查器没有价值"）**。初版 L09（元字符泄漏）报出 20+ 条，逐条排查后**全部是检查器自己的错**：<br>**① 把批处理语法当数据**：`chcp 65001 >nul`、`if not "%~1"=="" (`、`) else (`、`for /f ...` 的重定向与括号被当成"未加引号的元字符"。<br>**② 引号计数没扣除变量替换 token**：`set SRC_FILE="%SRC_FILE:"=%"` 里 `%VAR:"=%` 的引号**是该 token 语法的一部分**（误报 8 条）→ 先 `SUBST_TOKEN_RE` 替换再数引号。<br>**③ 循环修饰符被当成参数引用**：`for %%A in (%SRC_FILE%) do set SRC_SIZE=%%~zA` 中 **`%%~zA` 是 for 修饰符而非 `%~1`**，被展开成含元字符的样例值（误报 6 条）→ 单独用 `FOR_SAMPLE` 展开。<br>**④ 引号携带状态不能全仓库共用**：`convert_from_list_*.bat` 的 `SRC_FILE` 是**裸值**、`ffmpeg_*.bat` 是**带引号** —— 全仓共用会让 list 族被套上 ffmpeg 族性质（误报 8 条）→ 改为**按文件**统计（`quoted_vars_per_file`）。<br>**⑤ 变量跟踪不是顺序敏感的**：`SRC_BITRATE` 先是探测命令行、后被 `set /p` + `set /a` 覆盖成数字，全文件单一状态必然算错 → 升级为**逐行跟踪**（`quoted_vars_by_line`：后写覆盖先写；`set /p`/`set /a` 一律视为裸值；helper 输出参数按 `lib/common.bat` 的 `:extract` 契约标注第 N 个参数带引号）。<br>**L09 最终设计 = 差分式扫描**：同一行算两遍 —— *neutral*（变量替换为**空串**，保留行本身的 `&` `(` `>`）与 *worst case*（替换成含 `& | ( ) ^ !` 的样例值），**只有「worst 里活跃、neutral 里不活跃」的 `&`/`|` 才算缺陷**；`>` `<` `(` `)` 一律不参与判定（批处理里语法用途太多）。<br>**检查器自校准**：`calibrate_scanner()` 用三条自包含探针 —— `set X=%QVAR%` 必须干净、`set "X=%QVAR%"` 必须被抓、`endlocal & set X=%QVAR%` 必须干净；自校准失败即 SKIP，不假装 PASS。**这一步当场抓到"探针自己写错"**：最初用 `.replace('set "X=', 'set X=')` 造"好行"，只去掉开头引号、结尾引号还在 → 该行反因奇数引号被标记，自校准永远 False。 | (本次) |
| ✅ 已完成 | **新增 `test/lint/selftest.py`：检查器的双向验证（recall + precision），13/13 全绿**。"只会输出全部干净的检查器没有价值" —— 光看 `21 PASS` 无法区分"仓库真干净"与"检查器是瞎的"。selftest 用 13 个合成仓库同时验证**该抓的必须抓到（recall）**与**不该抓的必须沉默（precision）**；6 个 precision 用例就是本轮开发中**真实出现过的假阳性**（`%VAR:"=%` 引号计数、`endlocal & set` 字面量、`%%~zA` 循环修饰符、`set /p` 覆盖变量、安全的 `set VAR=%QVAR%`）。首跑 3 FAIL **全在夹具自身**（`lib/common.bat` 夹具用 `exit /b 9` 超出白名单、L01 用例的 CRLF 标志写反、L06 用例写在了被检查器跳过的 `echo` 行上），修夹具后 13/13。 | (本次) |
| ✅ 已完成 | **`ffmpeg_av1_nvenc.bat:169` 段注释表名写错（由 P02 对等检查抓出）**。该段注释写着 `码率查表: lib\bitrate_table_hevc.csv`，实为复制粘贴残留 —— 代码本身调用的是 `bitrate_table_av1.csv`（正确），**只有注释错**，其余 5 个 bat 均正确。值得一提的**发现路径**：P02 最初被这个注释**误报**成"av1_nvenc 接了两张表"；排查后确认**检查器太天真**（用 `name in 文本` 扫全文件，段注释里的表名也算命中），于是修检查器的同时也**修掉了这个真 bug** —— `table_of()` 改为优先匹配真实 `lookup_bitrate` 调用点、并**忽略注释行**（否则一条过期的 `rem` 就能伪造接线、把 P02 骗过去）。 | (本次) |
| ✅ 已完成 | **退出码语义两族统一（3 / 5）+ 补齐 `ffmpeg_libx264.bat` + 修 bat 低码率分支**（`AskUserQuestion` 确认的三项）。<br>**① 退出码契约统一为 bat 语义**：`0` 成功 / `1` 参数与文件错误 / `2` 查表越界 / `3` **无视频流**（`check_isvideo` 拦下）/ `5` **码率异常**。此前 sh 族与 bat 族的异常码不一致，跨族脚本无法统一判定；现由 L13 静态检查 + sh 冒烟 T 用例双向钉住。<br>**② `ffmpeg_libx264.bat` 补齐**：两族对等审查发现的**唯一真入口缺口** —— `.sh` 侧早就有 `ffmpeg_libx264.sh`（软编 H.264 保底），`.bat` 侧没有。补齐后**共享入口 11 → 12**；`lib/common.bat` 的 `lookup_bitrate` 本就支持 `bitrate_table_avc.csv`，无库改动，diff 与 `ffmpeg_libx265.bat` 骨架同构。<br>**③ bat 低码率分支**：原先两个 `if` 带交互限制，与 sh 侧行为不一致 → 去掉限制、改为直接走异常路径，并**补上层测试**（该分支此前无任何测试覆盖）。 | (本次) |
| ✅ 已完成 | **bat 探针补硬件门控 + 探针夹具规格修正，恢复 3 个静默丢失的硬件用例**。<br>**① `smoke_ffmpeg_bat.bat` v8 新增 `:gate` / `:skipcase`**（与 sh 侧 `gate_arg` 对等）：落在 T1/T2/T3/T7/T14 五个依赖硬件的用例，先用 320×240 小片试编一次，失败则记 `[SKIP] hardware absence, not a repo defect` 而非 FAIL → 无硬件的机器跑套件不再冒出**无意义 FAIL**（跨机器噪声源）。<br>**② 探针夹具 128×128 → 320×240**：128×128 下 `hevc_nvenc` / `av1_nvenc` 报 `InitializeEncoder failed: invalid argument`（**160×120 起正常**，QSV 无此限制），导致**有 N 卡的机器**把硬件用例误记 SKIP；修正后 sh 套件同一次全量运行从 `PASS=19 / SKIP=7` 变为 **`PASS=22 / SKIP=4`**，恢复的正是 `T2 ffmpeg_hevc_nvenc` / `T14 ffmpeg_av1_nvenc`（断言到 `codec=av1`）/ `T20 ffmpeg_hevc_nvenc_cygwin`。<br>**该项由"深测 PROBE-OK vs 冒烟 SKIP"的交叉比对抓出，是本轮最有价值的发现** —— 同一入口在深测里 320×240 真跑成功（写出 90 帧），在冒烟里却 SKIP；两个假象（探针尺寸不足 + 本机 `nvidia-smi` 坏）互相加固，让"有硬件却静默不测"长期看不出问题。详见 environment_matrix.md 待验证清单 19。 | (本次) |
| ✅ 已完成 | **能力报告深测阶段的 MSYS 路径改写缺陷（新坑，sh + bat 两侧同修）**。深测的 list 族全部 rc=1 `Error opening input`。日志显示入口把相对名 `clip.mp4` 规范化成了 `/c/Users/.../clip.mp4`，而**原生 `ffmpeg.exe` 打不开 POSIX 形式路径**（文件确实存在，极易误判成"入口坏了"）—— 根因是 **MSYS 只改写「命令参数」里的路径，不改写「作为值传入」的路径**。修复：① 深测前 `cygpath -m` 把工作目录规范成 `C:/...`；② 清单改用**绝对路径**，不再写相对名。该坑已与"MSYS2 ffmpeg 输出是 CRLF"并列写入 `environment_matrix.md` 表 4 的跨环境硬约束。 | (本次) |
| ✅ 已完成 | **测试文件名对等 + sh 侧补齐元字符矩阵（2026-09-16 深夜）**。用户指出：T 编号对等之后，**文件名层面也不对等** —— bat 侧 4 个套件、sh 侧只有 2 个，且名字对不上（`smoke_ffmpeg_bat.bat` ↔ `smoke_sh.sh`）。修法是**去掉族后缀、同名文件互为孪生**（目录名即族名）：`smoke_ffmpeg.sh`/`smoke_ffmpeg.bat`（各为原 `smoke_sh.sh` / `smoke_ffmpeg_bat.bat` 改名）、`smoke_special_chars.sh`/`smoke_special_chars.bat`、`smoke_all.sh`/`smoke_all.bat`（sh 侧两个均**新增**）、`check_env.sh`/`check_env.bat`。<br>**新增 `smoke_special_chars.sh`**（与 bat 同构：part A/C/B/D/Z、A 段同 20 个文件名），三处**刻意分歧**均在头注释写明：<br>① C/B 段用 `ffmpeg_libx264.sh` / `convert_from_list_libx265.sh` 软编入口 → **整套不需要任何硬件**（bat 侧用 QSV 是因为 Windows 测试机上它总在）；<br>② **A07 脱字符在 sh 侧是真用例**（bash 不二次解析参数），bat 必须 SKIP（`CALL` 会把 `^` 翻倍，探针造不出同名文件）；<br>③ part D 无参 stdin 在 sh 侧**真 PASS**（文件重定向确实到达 `read`；bat 侧是 cp65001 重启的探针取证限制）。part Z 定位为钉住 `run_list` 与后缀剥离依赖的 shell 构造（`${name%.*}`、`while IFS= read -r`），另加 **Z3 反斜杠文件名**（Linux 合法、Windows 文件系统造不出 → 条件 SKIP）。<br>**实测**：`PASS=28 FAIL=0 SKIP=1`（SKIP 仅 Z3，本机是 Windows/MSYS2 属预期）；A 段 20/20、C 段 3/3（codec=h264 断言）、B 段 list 6/6、Z1/Z2 PASS。<br>**过程中又抓到一个环境细节**：`$TMPDIR` 在本机是**反斜杠的 Windows 形式**（`C:\Users\...`），与 `/ffmpeg_bat_chars_sh` 拼接后成混合分隔符路径 —— 沙箱的 safe-delete 包装无法规范化它，`rm -f` 被拦成 fail-closed，使 D0 的预清理失效并撞上 A02 的同名产物。修法：两个 sh 套件启动时用 `cygpath -m` 把 WORK 归一化成**纯正斜杠绝对路径**，D0 改用独立文件名让"预清理"不再承重。该坑与"MSYS 只改写命令参数里的路径"同族，已入 `test/README.md`。<br>**沙箱权限复核**：用户称已授予完全权限，但实测 **cmd.exe 调用仍被拦截（Bash 与命令行工具两条通道都是）**、外部 exe 的重定向输出仍为 0 字节（whoami/Start-Process 双探针）→ `.bat` 侧验证仍需用户双击。 | (本次) |
| ✅ 已完成 | **文档同步**：`readme.md` 目录清单补 `test/lint/*` 与 `test/{sh,bat}/check_env.*`，`.bat` 编码器表补 `ffmpeg_libx264.bat`，Linux 用法补 `./ffmpeg_libx264.sh`，「验证状态」整节改写为表格 + 分层说明（含假 SKIP 修复说明）；新增「测试体系（三层，各管一件事）」小节含命令表。`environment_matrix.md` 新增待验证清单第 19 条（NVENC 最小尺寸 / bat 门控 / `NO-DEVICE` 假阴性 / MSYS 路径改写 / 两套新工具）与表 4 的两条跨环境硬约束。`test/README.md` 全量重写为 7 节（三层总览与快速上手、目录结构与约定、第一层 L/P 检查表 + L09 机制 + selftest + 退出码、第二层 T1–T25 逐项 sh/bat 覆盖对照 + SKIP 策略 + 320×240 依据、第三层状态词表 + 两个实现坑、两族对等矩阵、实测记录 + 待人工补跑清单 + 约定与坑 8 条）。 | (本次) |

| ✅ 已完成 | **负退出码陷阱 —— 两族失败路径对等的真正闭环 + remux moov 前置（2026-09-17，用户复跑探针后揪出）**。<br>**① 现象**：复跑 `check_env.bat /probe`，`ffmpeg_av1_qsv.bat` 仍是 `PROBE-FAIL rc=0 but no real output (0B) \| ERRORLEVEL:-40` —— 探针判 FAIL（判产物生效），但**入口自身仍返回 0**，即上一轮"失败传回 1"的修复在 `.bat` 侧**等于没生效**。<br>**② 根因（实测）**：cmd 的 `if errorlevel N` 读作「≥ N」且**按有符号比较**，而 Windows 版 ffmpeg 失败时返回**负** AVERROR。`subprocess` 取原始 32 位码三种典型失败：`av1_qsv` = `0xFFFFFFD8`(**-40** `ENOSYS`)、输入文件不存在 = `0xFFFFFFFE`(**-2** `ENOENT`)、参数错误 = `0xFFFFFFEA`(**-22** `EINVAL`) —— 即**所有真实失败路径**都被 `if errorlevel 1` 漏掉，守卫不触发 → 走完尾部 `echo ... 转换已出错或完成` → `exit /b 0`（日志里那行 `ERRORLEVEL:-40` 正是尾部 echo 打出来的）。`.sh` 侧 `[ $? -ne 0 ]` 一直正确，缺口只在 `.bat`。<br>**③ 修法**：8 个 `.bat` 入口统一改 `set "FB_RC=%ERRORLEVEL%"` + `if not "%FB_RC%"=="0" ( ... exit /b 1 )`（**字符串**相等比较，负数/正数/空值一律判失败；`if errorlevel` 在值为空时还会报语法错误后继续往下跑）。清单 wrapper 保留 `if errorlevel 1` 是**正确**的：其判定位于 `for /f` 块内，`%VAR%` 会被一次性展开冻结，而 `if errorlevel` 读实时值，且子进程受 L13 约束只返回 `{0,1,2,3,5}`。<br>**④ lint / selftest**：**L15 加硬** —— `.bat` 入口的失败守卫必须"负数安全"（接受 `if not "%X%"=="0"` 与 `%X% NEQ 0`，`if errorlevel N` 直接 FAIL）；selftest 18 → **22 例**（新增 `if errorlevel 1` recall 例、`%ERRORLEVEL% NEQ 0` 被接受的 precision 例、外加 moov 前置的 recall/precision 各一）。<br>**⑤ 探针补第三重判据**：两族深测要求 **rc=0 且 `run.log` 无 `Conversion failed`**（bat 另要求真实产物 >4096B）—— 三层互相独立，任一层被绕过另两层仍会说真话。<br>**⑥ remux moov 前置（用户要求）**：`ffmpeg_copy_to_mp4.{bat,sh}` 补 `-movflags +faststart`；实测同夹具不加 = `ftyp/free/mdat/moov`、加了 = `ftyp/moov/free/mdat` 且**字节数完全相同**（就地搬索引），`.sh` 真跑确认 moov 在第二位 + 双音轨 eng/chi 全保留；新增 **L17** 钉住两个 remux 出口。11 个编码入口仍为默认布局（待裁定）。<br>**⑦ 纠错**：上一轮把沙箱 bash 报的 `rc=127` 当成"ffmpeg 真返回 127"，据此得出「必须写 `if errorlevel 1`」——方向正好反了（127 只是 MSYS 对同一负码的渲染）。教训：**跨运行时的退出码必须先取原始 32 位值**。<br>**基线**：`lint 25 PASS / 0 FAIL / 5 WARN`、`selftest 22 cases / 0 FAIL`；L15/L17 均做了召回验证（临时撤掉修复即报 FAIL）。`.bat` 侧守卫**尚未真机运行**（沙箱无 cmd.exe），待用户双击验证 | 本次 |

**验证状态**：全部 14 个 .sh 重构完毕，eval 与 if-elif 查表在 .sh 中清零；试点+推广脚本在 Cygwin / MSYS2 mingw64 / 原生 gyan 三套 ffmpeg 下端到端验证通过（Cygwin 的 QSV 与 libx264/libx265 为构建限制，见矩阵文档）。**C 机 Linux（2026-09-16 第二轮）：15 个 sh 入口 15/15 全部实测（含 3 个无 N 卡预期失败路径），一键套件 `smoke_all.sh` PASS=18/18。**
