...
::set "RUN_COM="%FFMPEG_PATH%" -hide_banner -threads 0 -v verbose -init_hw_device qsv=hw -filter_hw_device hw"
::set "RUN_COM="%FFMPEG_PATH%" -hide_banner -threads 0 -init_hw_device qsv=hw:0 -filter_hw_device hw -hwaccel qsv -hwaccel_output_format qsv"
...
if defined BIT set "RUN_COM=%RUN_COM:&=^&% -c:v h264_qsv -profile:v main -preset veryfast -b:v %BIT% -g 250 -keyint_min 25 -ar 44100 -b:a 128k -c:a aac -ac 2 -map_metadata -1 -map_chapters -1 -strict -2 -rtbufsize 120m -max_muxing_queue_size 1024"
