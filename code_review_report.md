...
| 🔲 待办 | 4 个 list 驱动 bat（convert_from_list_qsv/cuda/libx265、repack_from_list）第 14 行 `for /f "delims=" %%i in (%SRC_FILE%) do <enc>.bat "%%i"` **缺 `call`**：按 cmd 链式语义，不带 `call` 调用另一 bat 不会返回，for 循环实际只处理清单第一项；且 `<enc>.bat` 为裸名调用（要求 cwd = 仓库目录）。探针 T9/T11 取证后修（`call "%~dp0<enc>.bat" "%%i"`） | — |
| 🔲 待办 | `-map 0:a` 未加 `?`：无音轨输入（屏幕录制/延时摄影）会被 ffmpeg 判为 `Failed to set value '0:a' for option 'map'` 直接失败（.bat/.sh 皆然，原版即有）。候选改 `-map 0:a?`；探针 T10 以 INFO 模式记录该行为 | — |
