...
#RUN_COM="${RUN_COM} -c:v av1_nvenc -preset p4 -tune:v hq -rc cbr -b:v $TARGET_BITRATE -g 250 -keyint_min 25 -ar 44100 -b:a 128k -c:a aac -ac 2 -map_metadata -1 -map_chapters -1 -strict -2 -rtbufsize 120m -max_muxing_queue_size 1024 -n \"${TARGET_FILE}\""
RUN_COM="${RUN_COM} -c:v av1_nvenc -preset p4 -tune:v hq -rc cbr -b:v $TARGET_BITRATE -g 250 -keyint_min 25 -ar 44100 -b:a 128k -c:a aac -ac 2 -map 0:v -map 0:a -map 0:s? -c:s mov_text -map_metadata 0 -map_chapters 0 -rtbufsize 120m -max_muxing_queue_size 1024 -n \"${TARGET_FILE}\""
