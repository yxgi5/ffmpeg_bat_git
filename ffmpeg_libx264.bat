...
rem (-profile:v high -preset fast -pix_fmt yuv420p)
...
set "BIT="
call "%SELF_DIR%lib\common.bat" lookup_bitrate %SRC_PIX% BIT bitrate_table_avc.csv
if not defined BIT (
...
if defined BIT set RUN_COM=%RUN_COM% -c:v libx264 -profile:v high -preset fast -b:v %BIT% -pix_fmt yuv420p -color_range tv -colorspace bt709 -color_primaries bt709 -color_trc bt709 -g 250 -keyint_min 25 -sws_flags bicubic -ar 44100 -b:a 128k -c:a aac -ac 2 -map 0:v -map 0:a? -map 0:s? -c:s mov_text -map_metadata 0 -map_chapters 0 -rtbufsize 120m -max_muxing_queue_size 1024
