...
**遗留事项**：.bat 家族重构与 T1–T13 冒烟已完成；第六轮 6b 的 `set` 写法修正已由探针 v6b 复核通过（A 18 PASS + 1 SKIP / C 3/3 / B 6/6 / D SKIP / Z1 PASS）；**但 6b 动了 5 个文件的核心路径，需再跑一次 T1–T13 套件做回归**（6b 探针只覆盖 `copy_to_mp4` + `avc_qsv` + `convert_from_list_qsv`，`hevc_nvenc`/`hevc_qsv`/`libx265`、另 3 个 list bat、banner/debug/`check_isvideo` 断言仍只有 T 套件覆盖）；Linux（A 机）与 Ultra 265K（C 机）平台实测（见 environment_matrix.md）；README/使用说明。
